...
type PushNotify struct {
	...
	IosProduct int `json:"iosProduction,omitempty"`
	...
	*/
	...
	SkipOnline int `json:"skipOnline,omitempty"`
	// 推送时，不走厂商通道1: 开启 默认: 其它,关闭
	SkipFactory int `json:"skipFactory,omitempty"`

	// 自定义内容, type=2
	CustomNotify *CustomNotify `json:"customNotify,omitempty"`

	// JSON格式 保留字段可参考下面附加参数示例(额外参数)
	ExtrasMapList []PushMap `json:"extrasMapList,omitempty"`
}
...
type AndroidNotify struct {
	...
	AndroidBadgeType string `json:"androidBadgeType,omitempty"`
	// 需要设置/累加的角标的值
	AndroidBadge string `json:"androidBadge,omitempty"`
}
